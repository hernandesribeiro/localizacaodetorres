...
label {
    ...
    color: #ADADC7 !important; /* Cor mais escura para dar ênfase */
}
</style>
...
try:
    ...
    # --- LAYOUT DE INPUTS PRIMÁRIOS ---
    col_concessao, col_lt,col_terminal, col_comprimento,col_fase, col_metodo, col_km_input = st.columns([1.2, 1.2,1.5,1.1,1.1,1.1,1.2]) 
    ...
    # --- LÓGICA DO TERMINAL A e COMPRIMENTO ---
    ...
    # --- LAYOUT DE INPUTS SECUNDÁRIOS ---
    #st.markdown("---")
    
    #col_fase, col_metodo, col_km_input, col_gap_3 = st.columns([0.8, 1.5, 1.5, 2.2])
    ...
    with col_km_input:
        valor_busca = st.number_input(
            ...
            format="%.2f",
            value=0.0,
            help=f"O valor de KM informado é medido a partir do Terminal A: {terminal_a}"
        )
...
""")
                ...
                # --- Tabela da Janela de Inspeção (LÓGICA DO 1% PARA TW ATUALIZADA) ---
                with col_tabela:
